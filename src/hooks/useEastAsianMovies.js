...
//Fetch data from TMDB API and update store
  ...
  const getEastAsianMovies = async () => {
    ...
    for (let i = 0; i < 3; i++) {
      const data = await fetch(`https://api.themoviedb.org/3/discover/movie?include_adult=false&include_video=false&language=en-US&page=1&sort_by=popularity.desc&vote_average.gte=7.751&vote_average.lte=8.494&vote_count.gte=1400&with_origin_country=${countries[i]}`, API_OPTIONS);
